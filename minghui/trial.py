...
import datetime
sys.path.append(os.path.abspath("../GrpB_models/b1_customer_preferences"))
import models
...
FUNCTIONS = {
    ...
    "Customer Preference Prediction": "cus_pref_model.pkl", #Question 6
...
if function_choice:
    ...
    if function_choice == "Customer Preference Prediction":
        ...
        MODEL_PATH = os.path.join(SCRIPT_DIR, FUNCTIONS[function_choice])
        if not os.path.exists(MODEL_PATH):
            st.error(f"❌ Model file `{FUNCTIONS[function_choice]}` not found!")
            st.stop()
    ...
    ############################ Question 7 ###################################
    elif function_choice == "CTR-Based 'Real-Time' Campaign Optimizer":
